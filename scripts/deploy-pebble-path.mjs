...
const source = await readFile(resolve(process.cwd(), "contracts/PebblePath.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "PebblePath.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["PebblePath.sol"]?.PebblePath;
if (!contract) {
...
console.log(`Deploying PebblePath from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_PEBBLE_PATH_CONTRACT_ADDRESS to this contract address in Vercel.");
